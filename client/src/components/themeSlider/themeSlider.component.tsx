...
export const ThemeSlider = () => {
    ...
    return (
      <div className="flex gap-3 items-center">
        <svg
          className={`w-5 h-5 ${enabled ? "text-white" : "text-primary-gray"}`}
          viewBox="0 0 17 16"
          fill="none"
        ...
        <Switch
          ...
          className={`bg-primary-gray relative inline-flex h-6 w-12 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75`}>
          <span className="sr-only">Use Dark Mode</span>
          <span
            aria-hidden="true"
            className={`${!enabled ? "translate-x-7" : "translate-x-1"}
            pointer-events-none inline-block h-3 w-3 transform rounded-full bg-white shadow-lg ring-0 transition duration-200 ease-in-out mt-1`}
          />
        </Switch>
        <svg
          className={`w-5 h-5 ${enabled ? "text-primary-gray" : "text-white"}`}
          viewBox="0 0 18 18"
          fill="none"
          xmlns="http://www.w3.org/2000/svg">
          <path
